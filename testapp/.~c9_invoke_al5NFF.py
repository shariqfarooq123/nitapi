...
def get_assigns(request,sub_id):
    return HResponse(functions.get_assignments_of_subject(sub_id), content_type="application/json" , status=status.HTTP_200_OK)
...
@api_view(['GET'])
@renderer_classes((JSONRenderer,))
def get_sub_detail(request,sub_id):
    assignments_json = functions.get_assignments_of_subject(sub_id)
    notes_json = functions.get_notes_of_subject(sub_id)
    assignments = json.loads(assignments_json)
    notes = json.loads(notes_json)
    data_detail = [assignments,notes]
    data_detail_json = json.dumps(data_detail)
    return Response(data_detail_json,content_type="application/json")
